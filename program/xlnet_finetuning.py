...
def convert_examples_to_features(examples, tokenizer, max_seq_length):
    ...
    for example_index, example in enumerate(examples):
        ...
        for index, statement in enumerate(example.statements):
            if '/' in statement:
                tmp = statement.split('/')
                statement = tokenizer.tokenize(tmp[0]) + ['[SEP]'] + tokenizer.tokenize(tmp[1])
            else:
                tokens = tokenizer.tokenize(statement)
            # 过长的token序列直接截断
            if len(statement.split()) > max_seq_length:
                statement = ""
                for token in tokens[:max_seq_length-3]:
...
def get_train_dataloader(train_features, train_batch_size):
    all_input_ids = torch.tensor([[choice["input_ids"] for choice in f.choices_features] for f in train_features])
...
def evaluate(model, device, eval_dataloader, desc="Train"):
    ...
    # """
    if desc == "Dev":
        with open("../result/xlnet_finetuning.csv", "w") as f_pred:
        ...
        with open("../result/xlnet_finetuning_logits.txt", "w", encoding="utf_8") as f_logit:
            with open('../dataset/test_bert.txt', "r", encoding="utf-8") as fin:
                for i, line in enumerate(fin.readlines()):
                    newline = eval(line)
                    logit = eval_logits[i]
...
hyperparameters = {
                    "max_sent_length": 30, "train_batch_size": 64, "eval_batch_size": 64,
                    "learning_rate": 3e-5, "num_epoch": 300, "seed": 54,
                    "patience": 1, "gradient_accumulation_steps": 4
                    }
